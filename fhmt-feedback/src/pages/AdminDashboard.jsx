...
const DEFAULT_PARTS = [
  ...
  {id:"part1",title:"Part 1",subtitle:"清信院前院與主院",icon:"🏯",group:"A",description:"情境：你從首頁走進「清信院」，先在前院看看，再走進主院（房子裡面）。",sections:[
    ...
    {title:"1.4　整體感覺",scale:"easy",items:[{id:"1.4.1",text:"你看得出來畫面上哪些地方可以點"},{id:"1.4.2",text:"進出場景的動畫順暢、不會卡卡的"}]},
  ]},
  {id:"part2",title:"Part 2",subtitle:"坐下靜坐",icon:"🧘",group:"A",description:"情境：你走到清信院前院左邊的大樹下，想坐下來靜坐休息。\n（提醒：有幾題要坐滿 5 分鐘，請耐心等一下，可以分次測）",sections:[
    ...
    {title:"2.3　開始坐、暫停、重來",scale:"easy",items:[{id:"2.3.1",text:"按下播放鍵（▶）後，時間開始往下倒數"},{id:"2.3.2",text:"倒數的時候，外圈的圓圈會跟著時間一格一格繞"},{id:"2.3.3",text:"正在倒數時，「重置」鍵是灰色的、按不下去"},{id:"2.3.4",text:"先暫停以後，「重置」鍵才變成可以按"},{id:"2.3.5",text:"按「重置」後，時間回到你剛剛設定的數字（例如剛剛設 6 分鐘就回到 6 分鐘）"}]},
  ...
  {id:"part3",title:"Part 3",subtitle:"上品受誡臺（受戒）",icon:"🌟",group:"B",description:"情境：你在清信院前院，點那個圓形的儀器（渾天儀），進入「上品受誡臺」受戒。",sections:[
    ...
    {title:"3.5　整體感覺",scale:"easy",items:[{id:"3.5.1",text:"你不用別人教，就知道要「長按」才能受戒"},{id:"3.5.2",text:"長按 3 秒的時間長短覺得 OK，不會太久或太短"}]},
  ]},
  // ── 組 C：地仙日誌 ──
  {id:"part4",title:"Part 4",subtitle:"地仙日誌",icon:"📖",group:"C",description:"情境：你在主院點桌上那本書，打開「地仙日誌」，看自己的修行紀錄。",sections:[
    ...
    {title:"4.5　整體感覺",scale:"easy",items:[{id:"4.5.1",text:"翻頁、看紀錄的操作你覺得直覺"},{id:"4.5.2",text:"書本翻頁動畫順暢、不卡"}]},
  ]},
  // ── 組 D：方寸澄心 + 登山進度 ──
  {id:"part5",title:"Part 5",subtitle:"方寸澄心（每週功課）",icon:"✍️",group:"D",description:"情境：你在主院打開「方寸澄心」，這是每週要做一次的功課，從「問童子」或「繪浮生」二選一寫一篇。",sections:[
    ...
    {title:"5.5　整體感覺",scale:"easy",items:[{id:"5.5.1",text:"你知道這是每週要做一次的功課"},{id:"5.5.2",text:"填寫、送出的流程清楚"}]},
  ]},
  {id:"part6",title:"Part 6",subtitle:"登山進度",icon:"⛰️",group:"D",description:"情境：你在主院點牆上的畫作，打開「登山進度」，看自己修到第幾轉、累積多少功德。",sections:[
    ...
    {title:"6.5　整體感覺",scale:"easy",items:[{id:"6.5.1",text:"你看得懂自己修到哪、還差多少"},{id:"6.5.2",text:"操作直覺"}]},
  ]},
  // ── 組 E：導覽與個人功能 + 整體順暢度 ──
  {id:"part7",title:"Part 7",subtitle:"導覽與個人功能",icon:"🧭",group:"E",description:"情境：請你在網站各處走走，試試右下角的導覽鈕、右上角的個人選單，還有一些零碎功能。最後幫忙感受整體順不順。",sections:[
    ...
    {title:"7.5　整體順暢度（壓力測試）",scale:"speed",note:"請你在各個場景之間多走幾趟，感受順不順。",items:[{id:"7.5.1",text:"場景之間切換的速度"},{id:"7.5.2",text:"開關各種視窗的反應速度"},{id:"7.5.3",text:"整體有沒有卡頓、變慢、轉圈圈很久"}]},
  ]},
  // ── 組 F：後台（只給後台測試員）──
  {id:"part8",title:"Part 8",subtitle:"後台管理",icon:"🔧",group:"F",description:"情境：你有後台帳號。請從後台網址登入，測試會員管理、年費等功能。後台是用 Email + 密碼登入，和前台的 Google 登入不一樣。",sections:[
    ...
    {title:"8.7　整體感覺",scale:"easy",items:[{id:"8.7.1",text:"後台操作直覺、找得到功能"},{id:"8.7.2",text:"文字、按鈕都看得懂"}]},
  ]},
...
function GroupCodeSettings({ parts }) {
  ...
  (parts || []).forEach(p => {
    ...
    groups[p.group].push(`${p.title}・${p.subtitle}`);
  });
...
function Overview({ users, parts, onDelete }) {
  ...
  return (<div style={{ display: "grid", gridTemplateColumns: sel ? "1fr 1fr" : "1fr", gap: 20, alignItems: "start" }}>
        ...
        : users.sort((a, b) => getP(b).pct - getP(a).pct).map(u => {
          ...
          return (<div key={u.odName || u.nickname} style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button onClick={() => setSel(act ? null : u)} style={{ flex: 1, display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", background: act ? "rgba(139,90,43,.08)" : "rgba(255,255,255,.7)", border: `1.5px solid ${act ? "rgba(139,90,43,.25)" : "rgba(0,0,0,.06)"}`, borderRadius: 14, cursor: "pointer", textAlign: "left" }}>
              <Ring progress={p.pct} /><div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 600, color: "#5B3A1F" }}>{u.nickname || "匿名"}</div><div style={{ fontSize: 12, color: "#9a8a6e", marginTop: 2 }}>{[u.device, u.browser, u.group ? `組${u.group}` : null, u.role].filter(Boolean).join(" · ")}</div></div>
    ...
    {sel && (<div style={{ position: "sticky", top: 80, padding: "20px 22px", borderRadius: 14, background: "rgba(255,255,255,.75)", border: "1px solid rgba(0,0,0,.06)" }}>
      ...
      <p style={{ fontSize: 12, color: "#9a8a6e", margin: "0 0 12px" }}>{[sel.device, sel.browser, sel.group ? `組${sel.group}` : null, sel.role].filter(Boolean).join(" · ")}{sel.updatedAt && ` · ${new Date(sel.updatedAt).toLocaleString("zh-TW")}`}</p>
