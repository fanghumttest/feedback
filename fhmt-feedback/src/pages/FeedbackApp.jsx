...
import { loadQuestions, loadPasscode, saveFeedback, loadFeedback } from '../storage';
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(160deg,#f7f0e3,#ede3d0,#e6d8c1)',padding:20}}>
      <div style={{width:'100%',maxWidth:400,padding:'44px 32px',borderRadius:20,background:'rgba(255,255,255,.75)',backdropFilter:'blur(20px)',boxShadow:'0 8px 40px rgba(91,58,31,.08)',border:'1px solid rgba(255,255,255,.6)',textAlign:'center'}}>
        ...
        <input value={code} onChange={e=>setCode(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleSubmit()}
          placeholder="請輸入通行碼" type="password" autoFocus
...
function Freeform({answers,onAnswer}) {
  const fs=[{key:'best',title:'💚 最喜歡的地方',ph:'哪一個功能或體驗印象最好？'},{key:'improve',title:'💡 最想改善的地方',ph:'哪裡卡卡的？'},{key:'bugs',title:'🐛 遇到的問題',ph:'按了沒反應、畫面跑掉……有遇到才填'},{key:'other',title:'💬 其他想說的話',ph:'任何想法都歡迎'}];
  return(<div>
  ...
  return(<div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(160deg,#f7f0e3,#ede3d0,#e6d8c1)',padding:20}}>
    <div style={{width:'100%',maxWidth:480,padding:'40px 32px',borderRadius:20,background:'rgba(255,255,255,.75)',backdropFilter:'blur(20px)',boxShadow:'0 8px 40px rgba(91,58,31,.08)',border:'1px solid rgba(255,255,255,.6)'}}>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:16}}>
        <div><label style={{fontSize:13,fontWeight:600,color:'#6B4E2E',display:'block',marginBottom:4}}>暱稱 <span style={{color:'#c49000'}}>*</span></label><input value={nick} onChange={e=>setNick(e.target.value)} placeholder="隨意填" style={{width:'100%',padding:'10px 14px',borderRadius:10,border:'1px solid rgba(0,0,0,.12)',fontSize:14,background:'rgba(255,255,255,.8)',boxSizing:'border-box',outline:'none'}}/></div>
...
export default function FeedbackApp() {
  ...
  const saveT=useRef(null);const contentRef=useRef(null);

  useEffect(()=>{(async()=>{
    const pc=await loadPasscode(); if(!pc) setAuthed(true);
    const q=await loadQuestions(); if(q){setParts(q);setCur(q[0]?.id||null);}
    setLoading(false);
  })();},[]);
  ...
  const doSave=useCallback(async()=>{
    if(!uid)return;setSaveStatus('saving');
    await saveFeedback(uid,{...userInfo,answers,freeform,updatedAt:new Date().toISOString(),odName:uid});
    setSaveStatus('saved');setTimeout(()=>setSaveStatus(''),2000);
  },[uid,userInfo,answers,freeform]);

  useEffect(()=>{if(!uid)return;if(saveT.current)clearTimeout(saveT.current);saveT.current=setTimeout(doSave,1200);return()=>{if(saveT.current)clearTimeout(saveT.current);};},[answers,freeform,doSave,uid]);

  const handleStart=async(info)=>{setUserInfo(info);const u=`${info.nickname}-${info.device}-${info.browser}`;const ex=await loadFeedback(u);if(ex){setAnswers(ex.answers||{});setFreeform(ex.freeform||{});}setView('form');};
